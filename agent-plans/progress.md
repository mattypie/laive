...
- Remaining follow-up after this slice: publish the playback-coherence fix, reinstall through the published `npx laive-mcp` path, and re-run the live Session View matrix (`launch_scene`, `stop_track_clips`, `stop_all_clips`) to confirm the mirror now settles correctly in a real Live session.
