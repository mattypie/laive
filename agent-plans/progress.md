...
- Updated the Max sidecar source patch so `fpic` requests embedding on re-export and the ASCII banner remains visible in presentation mode, reducing the chance of a blank exported device UI when the image asset is not resolved.
