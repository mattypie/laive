...
- Reworked the Max sidecar source patch again so the deterministic in-Live device UI is a `jsui`-rendered fixed-width ASCII `laive` banner, while keeping `logo.png` packaged for docs and future Max rendering experiments.
