...
5. The staged patcher now includes bundled logo assets and uses a `jsui` renderer for the in-Live device UI, drawing `logo.png` first with a built-in ASCII fallback if the image cannot be loaded.
