...
class LaiveControlSurfaceTests(unittest.TestCase):
    ...
    def test_get_selection_reads_arrangement_context(self):
        self.surface.process_request(
            create_request(
                "call",
                target="select_clip",
                arguments={"clip_id": "clip:arrangement:track:2:index:1"},
                request_id="select-clip-selection-1",
            )
        )

        response = self.surface.process_request(
            create_request("get", target="selection", request_id="get-selection-1")
        )

        self.assertTrue(response["ok"])
        self.assertEqual(response["result"]["selected_track_id"], "track:2")
        self.assertEqual(
            response["result"]["selected_clip_id"],
            "clip:arrangement:track:2:index:1",
        )
        self.assertEqual(response["result"]["selected_clip_location"], "arrangement")
    ...
    def test_arrangement_clip_creation_and_duplication_controls(self):
        ...
        set_arrangement_clip_bounds = self.surface.process_request(
            create_request(
                "call",
                target="set_arrangement_clip_bounds",
                arguments={
                    "clip_id": "clip:arrangement:track:1:index:2",
                    "start_beats": 36,
                    "end_beats": 44,
                },
                request_id="arrangement-bounds-1",
            )
        )

        self.assertTrue(set_arrangement_clip_bounds["ok"])
        self.assertEqual(
            set_arrangement_clip_bounds["result"]["clip"]["id"],
            "clip:arrangement:track:1:index:2",
        )
